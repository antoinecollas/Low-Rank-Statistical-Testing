...
##############################################################################
# Riemaniann Statistics
##############################################################################
def Riemaniann_distance_matrix_textures(𝐗, args):
    # WARNING: Two dates only
    α, β, tol, iter_max, scale = args
    (p, N, T) = 𝐗.shape

    # Estimate textures and matrices
    (𝚺_1, δ, iteration) = tyler_estimator_covariance(𝐗[:,:,0], tol, iter_max)
    (𝚺_2, δ, iteration) = tyler_estimator_covariance(𝐗[:,:,1], tol, iter_max)
    𝛕_1 = np.diagonal(𝐗[:,:,0].conj().T@np.linalg.inv(𝚺_1)@𝐗[:,:,0]) / p
    𝛕_2 = np.diagonal(𝐗[:,:,1].conj().T@np.linalg.inv(𝚺_2)@𝐗[:,:,1]) / p

    i𝚺_1_sqm = np.linalg.inv(sp.linalg.sqrtm(𝚺_1))
    distance = α * np.linalg.norm( np.log( i𝚺_1_sqm @ 𝚺_2 @ i𝚺_1_sqm ) ) + \
...
def covariance_equality_Wald_gaussian_statistic(𝐗, args=None):
    ...
    if args is not None:
        if args=='log':
            return np.real(np.real(L - vec(Q).conj().T @ (np.linalg.inv(O)@vec(Q))))
        else:
